...
- O coordinator entrega o resultado terminal ao autor de uma Task Telegram. O
  ...
  LLM, Specification ou worker; Tasks legadas de conversa privada no formato
...
- Lease expirado em `FINALIZING` não autoriza assumir que Git/PR terminou nem
  ...
  verificável.
- A recuperação durável estende os mecanismos já existentes de idempotência,
  lease, fencing e AuditEvent: no startup, somente Tasks ainda em `NEW` voltam
