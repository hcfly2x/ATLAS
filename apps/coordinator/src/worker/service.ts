...
const activeExecutionStatuses = new Set<ExecutionStatus>([
...
export class WorkerService {
  ...
  /**
   * A lease expiry during Codex execution is ambiguous: the coordinator can no
   * longer accept mutations from that worker, but cannot prove its process has
   * stopped. Fence it and fail closed; never create another Execution here.
   */
  async reconcileExpiredActiveLeases(now = new Date()): Promise<number> {
    const candidates = await this.options.prisma.execution.findMany({
      where: {
        leaseExpiresAt: { lt: now },
        reconciledAt: null,
        status: {
          in: [ExecutionStatus.RUNNING, ExecutionStatus.TESTING, ExecutionStatus.CANCEL_REQUESTED],
        },
      },
      select: { id: true },
    });
    let reconciled = 0;
    for (const candidate of candidates) {
      if (await this.reconcileExpiredActiveLease(candidate.id, now)) reconciled += 1;
    }
    return reconciled;
  }

  private async reconcileExpiredActiveLease(executionId: string, now: Date): Promise<boolean> {
    const idempotencyKey = `execution:${executionId}:lease-expired-active`;
    return this.options.prisma.$transaction(async (transaction) => {
      ...
      const stage =
        ...
        execution.task.state === TaskState.CANCEL_REQUESTED
          ? "cancellation"
          : "lease_expired";
      const transitioned = await transaction.task.updateMany({
        ...
        data: { failureStage: stage, state: TaskState.FAILED, version: { increment: 1 } },
      });
      if (transitioned.count !== 1) return false;
      const fenced = await transaction.execution.updateMany({
        where: { id: execution.id, leaseExpiresAt: { lt: now }, status: execution.status },
        data: {
          failureStage: stage,
          leaseExpiresAt: null,
          leaseId: null,
      ...
      if (fenced.count !== 1)
        throw new WorkerConflictError("expired execution changed during reconciliation");
      ...
      await transaction.auditEvent.create({
        data: {
          action: "execution.lease_reconciled",
          actor: "SYSTEM",
          correlationId: idempotencyKey,
          idempotencyKey,
          payload: json({
