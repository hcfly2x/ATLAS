...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,#123126,var(--bg) 45%);color:var(--ink);font:14px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace}
    ...
    .states{grid-column:span 8}.costs{grid-column:span 4}.tasks,.audit{grid-column:span 7}.detail,.memory{grid-column:span 5}
    ...
    #locked{max-width:480px;margin:15vh auto}.hidden{display:none}@media(max-width:850px){.states,.costs,.tasks,.audit,.detail,.memory{grid-column:1/-1}}
  </style>
</head>
<body>
...
async function load(){const pid=$("project").value;const q=pid?"?projectId="+encodeURIComponent(pid):"";const [o,t]=await Promise.all([api("/dashboard/api/overview"+q),api("/dashboard/api/tasks"+q)]);
